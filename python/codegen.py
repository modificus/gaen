...
SET (scripts_codegen_SOURCES
  ${CMAKE_CURRENT_BINARY_DIR}/registration.h
<<files>>
)

IDE_SOURCE_PROPERTIES( "/" "${CMAKE_CURRENT_BINARY_DIR}/registration.h" )
<<ide_source_props>>
'''

REGISTRATION_TEMPLATE = '''\
//------------------------------------------------------------------------------
// registration.h - Autogenerated from script Entities and Components<<license>>
//------------------------------------------------------------------------------
...
class ScriptInfo(object):
    def __init__(self, cmpFullPath, includes):
        ...
        cppPath = cmpFullPath[cmpFullPath.index('/src/scripts/cmp')+len('/src/scripts'):].replace('/cmp/', '/cpp/').replace('.cmp', '.h')
        self.cppFullPath = bindir() + cppPath
        self.hFullPath = self.cppFullPath.replace('.h', '__api.h')
...
def registration_cpp_path():
    return posixpath.join(sys.argv[1], "registration.h")
...
def write_registration_cpp(script_infos):
    ...
    impl_includes = '\n'.join('#include "%s"' % si.cppFullPath.replace(bindir(), 'scripts') for si in script_infos)
    calls_cpp = '\n    '.join(sorted(registration_call_lines))
    reg_cpp = REGISTRATION_TEMPLATE % (impl_includes, calls_cpp)
